...
const MintButton = ({ buttonText, width, address }:any) => {
  const chainId = 1;;
  ...
  const mintPresale = async (contract:any) => {
    ...
    if (signer)
    try {
      const allowlistProof = await fetchAllowlistProof(1, contract.address, signer._address);
  ...
  const handleClick = async () => {
    ...
    if (!walletReady(signer, activeChain, desiredChain))
      return;
    ...
    // const contract = await getDCNTContract(address, DCNTContractType, sdk);
    ...
    const contract = new ethers.Contract(address, abi, signer);
      ...
      try {
        ...
        presaleEnd = Math.floor(presaleEnd.getTime() / 1000);
